...
def _sync_overview_evolution(src_index, route_files):
    ...
    for target in route_files:
        ...
        s = re.sub(r'\s*<style id="overview-evolution-sync">.*?</style>', '', s, flags=re.S)
        ...
        if s.count('<!--OVERVIEW_EVOLUTION_SYNC_START-->') != 1 or s.count('<section class="evolution"') != 2:
...
def _postprocess_site(site_dir, date):
    ...
    sync_targets = [site / (k + '.html') for k, _t in titles
                    if k not in ('index', 'cycle') and _has_own_modules(site / (k + '.html'))]
    import re as _re
    live_site = Path(SITE)
