...
def view_check(root,stage,d,route):
    ...
    try:
        ...
        sections_for_view = [section for section in contract['routes'][route]['sections']
                             if not (route == 'theme' and section['id'] in ('research', 'cognition'))]
        expected=[num+' '+section['title'] for num,section in zip('一二三四五六七',sections_for_view)]
        ...
        for claim in model['claims']:
            ...
            theme_hidden = route == 'theme' and (
                claim.get('section') in ('research', 'cognition') or
        ...
        try:
            ...
            for width in (1440,390):
                ...
                for route in ROUTES:
                    ...
                    if(row){const items=[...row.children].filter(x=>x.matches('.hero,.kpi'));const compactCycle=(document.body.dataset.route==='cycle'||document.title.includes('周期'))&&items.length===2&&items[0].matches('.hero')&&items[1].matches('.kpi')&&items[1].children.length===4;if(!compactCycle&&(items.length!==5||!items[0].matches('.hero')||items.slice(1).some(x=>!x.matches('.kpi'))))errors.push('rowA hero/four KPI contract');}
