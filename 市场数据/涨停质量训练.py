...
v6(2026-07-11,Qlib IC复检落地,依据=_学习/涨停质量_QlibIC对照_20260711.md):
- 评估层加逐日截面RankIC/ICIR/t值(与qlib calc_ic同口径,rank后pearson,无scipy依赖);
- 淘汰双门=区分度<3pp 或 |IC_t|<2(截面因子); 翻转判定改近20日RankIC符号(替代近窗rho,样本效率高一个量级);
- ★市场温度/涨停环境=日级时序因子(日内同值,截面选股信息=0)移出选股加权,改两端环境规则(冰点<25加分/过热≥85全场回避);
- 封单额标冗余出权重(与封单比日内rank相关0.54=同一信息双倍下注,留封单比)。
...
# ---------- v6 IC层: 因子数值映射(逐日截面RankIC用; None=日级时序环境因子,不参与截面IC与选股加权) ----------
def _fb_min(v):
    if not isinstance(v,str) or ':' not in v: return np.nan
    try:
        h,m=v.split(':'); return (int(h)-9)*60+int(m)-15
    except Exception: return np.nan
FNUM={
 '首封时间':('首封',_fb_min),'封单比':('封单比',None),'连板':('连板',None),'开板次数':('开板',None),
 '换手率':('换手',None),'流通市值':('市值亿',None),'股价档':('股价',None),'涨停基因10日':('基因',None),
 '量比':('量比',None),'60日位置':('位置',None),'板前5日动能':('动能',None),'行业共振':('行业家数',None),
 '题材共振':('题材家数',None),'封单额':('封额亿',None),
 '涨停环境':None,'市场温度':None,   # 日级同值,截面IC不存在
}
IC_MIN_T=2.0; IC_RECENT=20; REDUNDANT={'封单额':'冗余·与封单比同源r0.54(留观察不加权)'}
def _fac_ic(S,name):
    """逐日截面RankIC(因子值 vs E1执行收益): 当日有效对>=10才计入; 返回均值/ICIR/t/近20日均值。"""
    spec=FNUM.get(name)
    if spec is None: return None
    col,fn=spec
    v=S[col].map(fn) if fn else pd.to_numeric(S[col],errors='coerce')
    d=pd.DataFrame({'日':S['日'].values,'v':v.values,'y':S['E1'].values}).dropna()
    ics=[]
    for day,g in d.groupby('日'):
        if len(g)<10: continue
        xr=g['v'].rank(); yr=g['y'].rank()
        if xr.std()==0 or yr.std()==0: continue
        r=float(np.corrcoef(xr,yr)[0,1])
        if r==r: ics.append((day,r))
    if len(ics)<20: return None
    s=pd.Series([r for _,r in ics],index=[dd for dd,_ in ics]).sort_index()
    m=float(s.mean()); sd=float(s.std()); icir=m/sd if sd>0 else float('nan')
    rec=float(s.iloc[-IC_RECENT:].mean()) if len(s)>=IC_RECENT else None
    return dict(RankIC=round(m,4),ICIR=round(icir,3),t=round(icir*len(s)**.5,2),
                近20日RankIC=round(rec,4) if rec is not None else None,IC天数=int(len(s)))
...
def build(fetch=False,stage=None):
    ...
    for name,(fn,order) in FACTORS.items():
        ...
        ic=_fac_ic(S,name)   # v6: 逐日截面RankIC
        ...
        if len(qual)>=2:
            ...
            if FNUM.get(name) is None:
                status='环境因子·不参与选股加权(日内同值,截面IC不存在)'   # v6: 温度/涨停环境→环境规则
            elif name in REDUNDANT:
                status=REDUNDANT[name]   # v6: 冗余因子留统计不加权
            elif disc<MIN_DISC: status='淘汰·区分度不足'
            elif ic is not None and abs(ic['t'])<IC_MIN_T: status=f"淘汰·IC不显著(|t|={abs(ic['t'])}<{IC_MIN_T})"
            else:
                w=disc*(0.5+0.5*abs(rho or 0)); status='有效'
                # v6翻转判定: 近20日RankIC符号与全窗相反且幅度>=0.01(替代近窗rho)
                if ic is not None and ic.get('近20日RankIC') is not None and ic['RankIC']*ic['近20日RankIC']<0 and abs(ic['近20日RankIC'])>=0.01:
                    status='翻转降权(近20日RankIC反号)'; w*=0.5
        ent.update(区分度=disc,单调rho=rho,近窗rho=rho_r,IC=ic,状态=status,原始权重=round(w,2))
        facs[name]=ent
    ...
    lib=dict(更新=datetime.date.today().strftime("%Y%m%d"),窗口=f"{P['日'].min()}~{P['日'].max()}",样本=int(len(S)),
        口径=("v6多因子评分卡(截面因子加权+IC显著性双门)。因子T日收盘可知;★执行口径:执1/执2买入点同为T+1开盘——执1=T+1开→T+1收(A股T+1制度当日卖不出,故执1=买入当天浮动盈亏/逐日盯市,检验开盘追买当天吃肉还是被套);执2=T+1开→T+2收(首个可完整执行的真实回合,最贴近交割的战绩口径)。荐票主口径=执行,一字开盘可能仍买不进=上界;"
             "信号口径T1/T2=T收→T+1/T+2收(仅参考)。不复权,零后视镜。质量分=Σ权重×桶分(0.5执1胜率+0.3执1均涨+0.2执2均涨映射);"
             f"有效性(v6)=区分度×单调性×IC显著性: 淘汰双门=区分度<{MIN_DISC}pp或|IC_t|<{IC_MIN_T}; 翻转=近{IC_RECENT}日RankIC与全窗反号降权50%; "
             f"市场温度/涨停环境=日级环境因子移出选股加权(见环境规则); 封单额冗余出权重。IC口径=逐日截面spearman(因子,E1),依据Qlib复检20260711。"),
    ...
    # v6 环境两端规则: 温度=日级环境开关(不进选股权重),两端才有信息(中间无梯度,QlibIC复检20260711)
    denv=S.dropna(subset=['温度']).groupby('日').agg(v=('温度','first'),ret=('E1','mean'))
    cold=denv[denv['v']<25]; hot=denv[denv['v']>=85]
    lib['环境规则']=dict(
        说明='市场温度/涨停环境为日级时序因子(日内同值,截面选股信息=0),v6起移出选股加权;两端规则每晚按日级等权重算。',
        冰点=dict(界='<25',天数=int(len(cold)),日均执1pct=round(float(cold['ret'].mean())*100,2) if len(cold) else None,动作='全场加分观察'),
        过热=dict(界='≥85',天数=int(len(hot)),日均执1pct=round(float(hot['ret'].mean())*100,2) if len(hot) else None,动作='全场回避·荐票仅观察'))
    ...
    snap=dict(日=lib['更新'],样本=lib['样本'],基准执1=[base['执1胜率'],base['执1均涨']],
        因子={k:dict(权重=e['权重'],区分度=e['区分度'],rho=e['单调rho'],状态=e['状态'],
            RankIC=(e.get('IC') or {}).get('RankIC'),IC_t=(e.get('IC') or {}).get('t')) for k,e in facs.items()})
    open(os.path.join(L,"_涨停质量库快照.jsonl"),"a",encoding="utf-8").write(json.dumps(snap,ensure_ascii=False)+"\n")
    md=[f"# 涨停质量库v6(IC显著性双门+环境规则) {lib['窗口']} 样本{lib['样本']} 零后视镜·执行口径",
        f"环境规则: 冰点{lib['环境规则']['冰点']} | 过热{lib['环境规则']['过热']}",
    ...
    for name in sorted(facs,key=lambda x:-facs[x]['权重']):
        e=facs[name]
        ics=(f" | RankIC{e['IC']['RankIC']:+} ICIR{e['IC']['ICIR']:+} t{e['IC']['t']:+} 近20日{e['IC']['近20日RankIC']}" if e.get('IC') else " | 截面IC:—")
        md.append(f"\n## {name} | 状态:{e['状态']} 权重{e['权重']} 区分度{e['区分度']}pp rho{e['单调rho']}"+ics)
        for k in e['桶序']:
            if k in e['桶']:
                v=e['桶'][k]
    ...
    print(f"质量库v6建成: 样本{lib['样本']} 窗口{lib['窗口']} 活跃因子{len(active)}/{len(FACTORS)}")
    for k in sorted(active,key=lambda x:-facs[x]['权重']):
        _ic=facs[k].get('IC') or {}
        print(f"  {k}: w{facs[k]['权重']} 区分度{facs[k]['区分度']}pp rho{facs[k]['单调rho']} t{_ic.get('t')} {facs[k]['状态']}")
    return lib
...
def 打标当日(d):
    ...
    for _,r in D.iterrows():
        ...
        rec=dict(代码=row['代码'],名称=row['名称'],首封=row['首封'],连板=row['连板'],开板次数=row['开板'],
            ...
            环境提示=('过热回避' if (row.get('温度') is not None and row['温度']==row['温度'] and row['温度']>=85) else '冰点加分' if (row.get('温度') is not None and row['温度']==row['温度'] and row['温度']<25) else None),
