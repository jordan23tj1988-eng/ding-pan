...
def _render(model,contract,root=None):
    ...
    def render_claim(key, as_item=False, tag_override=None):
        ...
        if (not c.get('legacy_html') and pointer.startswith('/bodies/')
            ...
            if key not in hidden_bank_ids:
                hidden_bank_ids.append(key)
    ...
    # 数据层一条不删 —— this is a visibility change, not a data change.
    # 主题页按既有契约完全隐藏(见 test_review_pages_hero_golden 的 theme 分支)。
    if route not in AUDIT_FOLD_HIDDEN_ROUTES:
        absent = [c for c in model['claims'] if ('id="claim-' + c['id'] + '"') not in body]
        if absent:
            body += '<div class="claim-anchor-bank" hidden aria-hidden="true">' + ''.join(
                '<span id="claim-' + e(c['id']) + '" data-role="' + e(c['role']) + '"></span>' for c in absent
            ) + '<pre class="claim-anchor-text">' + e(json.dumps(
                [{'id': c['id'], 'role': c['role'], 'text': c['text'], 'source_pointer': c.get('source_pointer')} for c in absent],
