...
def evo_panel(title, data, rows, key):
    st = (data or {}).get(key, {})
    pills=''.join('<span class="evo-stat"><b>%s</b><small>%s</small></span>' % (st.get(k,0),lab) for k,lab in [('inherited','历史承接'),('experiences','经验/教训/信号'),('capabilities','已沉淀能力'),('hits','后续命中'),('validated','验证成功'),('refuted','已证伪'),('tracking','追踪中'),('shelved','已搁置')])
    items=[]
    for r in (rows or []):
        if r.get('capitalized') is True or r.get('state') in ('capability','validated','refuted') or r.get('hit_count') is not None or r.get('validation') in ('validated','refuted'):
            items.append('<div class="evo-row"><b>%s</b><span>%s</span><small>验证：%s · 命中：%s · 资金化：%s</small></div>' % (r.get('route','—'),r.get('title') or r.get('body','—'),r.get('validation') or '待验证',r.get('hit_count') if r.get('hit_count') is not None else '—','是' if r.get('capitalized') is True else '—'))
    body=''.join(items) or '<div class="evo-empty">当前历史资料没有明确的能力化、后续命中或验证证据，暂不计数。</div>'
    return '<section class="evolution"><h2>%s<span class="hint">发现 → 追踪 → 验证 → 经验 → 能力 → 后续命中</span></h2><div class="evo-stats">%s</div><details open><summary>已沉淀与验证明细 <span class="chip">%d条有结果记录</span></summary><div class="evo-list">%s</div></details></section>'%(title,pills,len(items),body)
