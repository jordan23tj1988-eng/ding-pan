...
def evaluate(diag:Path, progress:Path, evidence_dir:Path|None=None):
    ...
    for d in DATES:
        ...
        items.append({'date':d,'status':status,'verdict':verdict,'verdict_path':verdict_path.relative_to(evidence_dir).as_posix() if verdict_path else None,'same_day_evidence_count':len(same_day),'evidence':hits[:8],'reason':reason})
