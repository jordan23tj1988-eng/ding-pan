...
★执行口径:T+1开盘买入→T+1收盘。产出:竞价池结算_{d}.json + 追加 _竞价池结算.jsonl + _竞价池反思.jsonl(按信号/连板/高开档归因,验证"低开有肉高开套") + 竞价池结算卡_{d}.html(嵌auction页终结算段)。
...
def main(dprev):
    ...
    out=dict(池日=dprev,结算日=dnext,明细=res,
        ...
        口径="★执行口径=T+1开盘买入→T+1收盘;池=发出版冻结名单,不可事后增删")
    json.dump(out,open(os.path.join(L,f"竞价池结算_{dprev}.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1)
    open(os.path.join(L,"_竞价池结算.jsonl"),"a",encoding="utf-8").write(json.dumps(dict(池日=dprev,**{k:v for k,v in out["汇总"].items() if k not in("按信号","按连板","按高开档")}),ensure_ascii=False)+"\n")
    open(os.path.join(L,"_竞价池反思.jsonl"),"a",encoding="utf-8").write(json.dumps(dict(池日=dprev,结算日=dnext,反思=refl,按信号=bysig,按连板=bylb,按高开档=bygk),ensure_ascii=False)+"\n")
    ...
    card=(f'<div class="card"><b>{disp}竞价池终结算(T+1执行口径):次日封板{seal}/{len(pool)},胜率{out["汇总"]["执行胜率"]},均收{top_avg}%'
      ...
      +'<table><tr><th>票</th><th>信号</th><th>T+1高开</th><th>执行收益</th><th>判定</th></tr>'+rows+'</table>'
      +f'<p><b>归因(A档):</b>按信号 一字{bysig["一字"]}% / 秒板{bysig["秒板"]}%;按连板 首板{bylb["首板"]}% / 2板+{bylb["2板+"]}%;按高开档 低开≤0:{bygk["低开≤0"]}% / 0~5:{bygk["高开0~5"]}% / ≥5:{bygk["高开≥5"]}%。</p></div>')
