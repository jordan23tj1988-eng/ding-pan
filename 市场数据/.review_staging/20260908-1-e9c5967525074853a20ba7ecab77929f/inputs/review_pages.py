...
def _components(model,root):
    ...
    def artifact(key,section,name,producer,anchors=None):
        ...
        if key=='FUNDTEMP' and not p.exists():
            ...
            row=next((x for x in table if str(x.get('日'))==d),None) if isinstance(table,list) else None
            if row:
                html=('<div class="card"><b>资金温度 · 分位</b> '+str(row.get('温度分位','—'))+
                      '<br><span class="mut">机构 '+str(row.get('机构席次','—'))+'席 / '+str(row.get('机构金额亿','—'))+'亿</span></div>')
