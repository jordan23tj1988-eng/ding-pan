...
def check_page(h, d):
    ...
    if has_body:
        ...
        if not miss_ids:
            ...
            chk(issues, ('class="steps"' in v) or ('<!--VOLSTEP-->' in v), '段一量能台阶组件在位', 'body台阶块或VOLSTEP卡')
            chk(issues, ('<svg' in l) or ('<!--LEADIND-->' in l), '段二先行指标图/卡在位', 'body图或LEADIND卡')
            chk(issues, ('<!--VOTEBOARD-->' in st) or ('<!--MACHVOTE-->' in st), '段三五路投票块在位', 'VOTEBOARD或MACHVOTE')
            chk(issues, ('class="cols"' in ld) or ('<!--LADDER-->' in ld), '段四连板梯队条在位', 'body梯条或LADDER卡')
            chk(issues, ('class="stages"' in ps) and ('class="posmeter"' in ps), '段五三态+仓位条在位', '')
