...
def _render(model,contract,root=None):
    ...
    if route not in AUDIT_FOLD_HIDDEN_ROUTES:
        ...
        for c in model['claims']:
            ...
            if text and text not in on_page and c not in absent:
                absent.append(c)
        if absent:
            # 锚点只补页面里没有的 id(避免 duplicate IDs)；原文一律留档。
            spans = ''.join('<span class="claim-anchor" id="claim-' + e(c['id']) + '"></span>'
                            for c in absent if ('claim-' + c['id']) not in present_ids)
