...
def card(d, t):
    ...
    # Re-render from date-bounded frozen facts, not a stale cached trigger list.
    trig = triggers(d, t)
    hit = {"ice": any("冰点进攻窗" in m for m in trig),
           "hot": any("过热禁追窗" in m for m in trig),
           "wash": any("洗出反弹窗" in m for m in trig)}
    if temp is None:
        hit["ice"] = hit["hot"] = None
    if len(prem) < 3 or any(p is None for p in prem[-3:]):
        hit["wash"] = None
    ...
    def lamp(on, name, icon, cond, rec, ctone):
        if on is None:
            return (f'<div style="flex:1;min-width:190px;border:1px solid #33383f;border-radius:8px;padding:8px 10px">'
                f'<div style="font-weight:700;font-size:13px;color:#5b636d">{icon} {name} · —</div>'
                f'<div style="font-size:11px;color:#5b636d">{cond}</div></div>')
        if on:
    ...
    def chip(on, txt, ctone):
        if on is None:
            return f'<span style="padding:2px 10px;border-radius:99px;border:1px solid #33383f;color:#5b636d">{txt}·—</span>'
        if on:
    ...
    strip = ('<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin:0 0 10px;font-size:12px">'
        ...
        + (("%.1f%%" % (j12[-1] * 100)) if j12 and j12[-1] is not None else "—")
        + ' · 昨停溢价 ' + (("%+.2f%%" % prem[-1]) if prem and prem[-1] is not None else "—") + "</span></div>")
