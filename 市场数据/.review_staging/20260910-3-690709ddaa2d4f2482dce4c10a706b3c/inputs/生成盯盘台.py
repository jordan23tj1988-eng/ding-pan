...
def _postprocess_site(site_dir, date):
    ...
    overview_ready = len(_re.findall(r'<section class="evolution">',
                                     (site / 'index.html').read_text(encoding='utf-8'), _re.S)) == 2
    live_site = Path(SITE)
    if sync_targets and overview_ready:
        _sync_overview_evolution(site / 'index.html', sync_targets)
    elif not overview_ready:
        # 概览页能力模块重建不属于候选站点阶段(现站由概览重建补齐)。候选站点缺它时
        # 不制造假数据：跳过通用同步，cycle/lhb 改用现站能力模块源，产物与到站一致。
        print('提示: 候选站点概览页无能力模块(概览重建在到站阶段)，跳过通用同步；'
              'cycle/lhb 使用现站能力模块源', live_site)
