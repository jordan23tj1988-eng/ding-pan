...
def main():
    ...
    # 主题页能力恢复：在五个业务板块之后，追加与周期/龙虎榜同构的两组
    # evolution 模块；业务 h2 仍只统计一至五，避免把能力段误算进主题契约。
    evo = re.findall(r'<section class="evolution">.*?</section>', h, re.S)
    chk(issues, len(evo) in (0, 2), '能力进化模块为0或2组', f'实际{len(evo)}组')
    if len(evo) == 2:
        for n, block in zip(('六', '七'), evo):
