...
def _deploy_site(date, result):
    ...
    # 通用能力模块同步仅适用于仍采用 section#research/cognition 的其他路；cycle 使用黄金版 h2 直连专属链
    sync_targets = [Path(SITE) / (k + '.html') for k, _t in titles if k not in ('index', 'cycle')]
    if sync_targets:
        _sync_overview_evolution(Path(SITE) / 'index.html', sync_targets)
    # cycle 黄金视觉锁: release 只提供当日数据，周期页形态由只读黄金版固定；六/七能力模块从现站/概览移植
    repair_scripts = [Path(BASE) / 'restore_cycle_page.py', Path(BASE) / 'restore_evolution_style.py']
    for repair in repair_scripts:
        if not repair.exists():
            raise RuntimeError('cycle visual repair script missing: ' + str(repair))
        cp = subprocess.run([sys.executable, str(repair)], cwd=BASE, capture_output=True, text=True, encoding='utf-8')
        if cp.returncode != 0:
            raise RuntimeError('cycle visual repair failed: ' + str(repair) + '\\n' + cp.stdout[-2000:] + cp.stderr[-2000:])
    # 龙虎榜黄金视觉锁：release 只提供当日数据，页面形态由黄金版固定；
    # 当前发布的自主拓展/认知迭代模块由恢复器原样保留，并由目标日模拟盘重接。
    ...
    cp = subprocess.run(
        [sys.executable, str(lhb_repair), date],
        cwd=BASE, capture_output=True, text=True, encoding='utf-8',
    )
    if cp.returncode != 0:
        raise RuntimeError(
            'lhb visual repair failed: ' + str(lhb_repair) + '\\n'
            + cp.stdout[-2000:] + cp.stderr[-2000:]
        )
