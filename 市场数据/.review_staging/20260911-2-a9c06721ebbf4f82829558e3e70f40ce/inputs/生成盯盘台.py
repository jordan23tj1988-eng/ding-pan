...
def _sync_overview_evolution(src_index, route_files):
    ...
    for target in route_files:
        ...
        # 旧 research/cognition 只保留在数据审计层，展示层必须由标准能力模块替换，
        # 否则每次复盘都会出现重复的“自拓展/认知迭代”区块。
        if not is_theme:
            s = re.sub(r'<section id="research"[^>]*>.*?</section>\s*', '', s, count=1, flags=re.S)
            s = re.sub(r'<section id="cognition"[^>]*>.*?</section>\s*', '', s, count=1, flags=re.S)
        ...
        if is_theme:
            ...
            placement_ok = i_res < 0 and i_evo >= 0
        if not placement_ok:
...
def _postprocess_site(site_dir, date):
    ...
    # 所有子路统一替换展示层旧 research/cognition；数据审计仍保留原始角色。
    sync_targets = [site / f'{key}.html' for key in ('auction','lhb','theme','logic','limitup','cycle')
                    if (site / f'{key}.html').is_file()]
    import re as _re
