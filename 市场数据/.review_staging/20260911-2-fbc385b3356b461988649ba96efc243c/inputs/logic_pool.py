...
def load_logic_candidates(d, L):
    """返回同一逻辑发出版中的观察候选，不进入正式荐票/结算。"""
    for name,strict in (('logic判断_%s.json'%d,True),('逻辑荐票_%s.json'%d,False)):
        ...
        if strict:
            return [x for x in items if str(x.get('类型','')).strip()!='荐票'], name
        return [x for x in (j.get('观察') or []) if isinstance(x,dict)], name
    return [], None
