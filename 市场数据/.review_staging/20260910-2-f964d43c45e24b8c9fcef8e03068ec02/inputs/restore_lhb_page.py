...
def build_page(d: str, current_html: str | None = None) -> str:
    ...
    if not SITE.exists(): raise RuntimeError(f"当前页不存在: {SITE}")
    golden = GOLDEN.read_text(encoding="utf-8")
    current = current_html if current_html is not None else SITE.read_text(encoding="utf-8")
    evo = _evolution_sections(current)
    if len(evo) < 2 and INDEX.exists(): evo = _evolution_sections(INDEX.read_text(encoding="utf-8"))
    ...
    head_source = current
    head_start = head_source.find('<div class="wrap">')
    head_end = head_source.find('<section id="seats"', head_start)
    if head_end < 0:
        for candidate in sorted(
            SITE.parent.glob("lhb.html.before_*.bak"),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        ):
            probe = candidate.read_text(encoding="utf-8")
            ps = probe.find('<div class="wrap">')
            pe = probe.find('<section id="seats"', ps)
            if ps >= 0 and pe > ps:
    ...
    if '<style id="overview-evolution-sync">' not in style_source and INDEX.exists():
        style_source = INDEX.read_text(encoding="utf-8")
...
def main() -> int:
    ...
    backup = SITE.with_name(SITE.name + f".before_lhb_restore_{d}.bak")
    shutil.copy2(SITE, backup)
    tmp = SITE.with_name(SITE.name + ".lhb_restore_tmp")
    tmp.write_text(out, encoding="utf-8", newline="\n")
    tmp.replace(SITE)
    print(f"restored {SITE} bytes={SITE.stat().st_size}")
