...
def build_page(d: str, current_html: str | None = None) -> str:
    ...
    if first_four >= 0 and (first_five < 0 or first_four > first_five):
        nxt = re.search(r"<h2>", dynamic[first_four + 5:])
        four_end = first_four + 5 + nxt.start() if nxt else len(dynamic)
        dynamic_front = (dynamic[:first_five].rstrip() + "\n" +
                         dynamic[first_four:four_end].strip())
    else:
        dynamic_front = dynamic[:first_five].rstrip()
    ...
    extra = dynamic[dynamic.find("<h2>五"):].lstrip().replace("<h2>", "<h3>").replace("</h2>", "</h3>")
