...
def r_judge(body):
    ...
    _first = re.search(r'<h2(?:\s[^>]*)?>\s*一\b.*?</h2>', body, re.S)
    if not _first: return ''
    i, e = _first.start(), _first.end() - 1
    h = body[i:e + 1]
    s = body.find('<!--SEATCARD-->', i)
    s_end = body.find('<!--/SEATCARD-->', i)
    _next = re.search(r'<h2(?:\s[^>]*)?>', body[e + 1:])
    _next_pos = e + 1 + _next.start() if _next else -1
    # 新格式：手写板块先于荐票标记，只保留板块一首段；旧格式：保留
    # SEATCARD 前包装并从其后截取到下一个 h2，避免跨版本吞页。
    if _next_pos > 0 and (s < 0 or _next_pos < s):
        return _rec2obs(body[i:_next_pos]) + chr(10) + r_seatcard(body, d)
    prefix = body[e + 1:s] if (s >= 0 and s > e) else ''
    rest_start = (s_end + len('<!--/SEATCARD-->')) if s_end > 0 else e + 1
    _after = re.search(r'<h2(?:\s[^>]*)?>', body[rest_start:])
    j = rest_start + _after.start() if _after else -1
    rest = _rec2obs(prefix + body[rest_start:j if j > 0 else len(body)])
    return h + chr(10) + r_seatcard(body, d) + chr(10) + rest
...
def r_cog(body, d=None):
    ...
    _delta = out.count('<div') - out.count('</div>')
    if _delta < 0:
        raise RuntimeError('lhb cognition contract failed: div close exceeds open')
    if _delta:
        out += '</div>' * _delta
    return out
...
def build_page(d):
    ...
    if page.count('<div') != page.count('</div>'):
        raise RuntimeError(
            'lhb body contract failed: div open=%d close=%d' %
