...
def _coverage(root, d):
    ...
    for path in paths:
        ...
        for index, item in enumerate(items):
            ...
            rows.append(row)
    if read_count != total:
        errors.append(f"消费者应答覆盖不足: {read_count}/{total}")
    return {"total_items": total, "responses_read": read_count,
            ...
            "status_counts": {s: sum(x["status"] == s for x in rows) for s in
                              ("true", "false", "pending", "unverifiable", "ambiguous", "missing")}}, rows, errors
...
def main(argv=None):
    ...
    if args.command == "coverage":
        result = audit_learning(args.root, args.d, args.out)
