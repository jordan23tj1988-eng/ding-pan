...
def inject(block):
    ...
    if not js:
        raise RuntimeError("无judgment，拒绝静默成功")
    jp=js[-1]; J=json.load(open(jp,encoding="utf-8")); a=J["bodies"]["auction"]
    start, end = "<!--POOLLEDGER-->", "<!--/POOLLEDGER-->"
    opens, closes = a.count(start), a.count(end)
    if opens != closes or opens > 1:
        raise RuntimeError(f"POOLLEDGER锚点不成对: open={opens} close={closes}")
    if opens == 1:
        a2, replaced = re.subn(r'<!--POOLLEDGER-->.*?<!--/POOLLEDGER-->',
                               lambda m:block, a, count=1, flags=re.S)
        if replaced != 1:
            raise RuntimeError(f"POOLLEDGER替换数={replaced}，拒绝静默成功")
        a = a2
    else:
        m=re.search(r'<h2[^>]*>(?:三|二) 昨日[^<]*</h2>',a)
        if not m: raise RuntimeError("段二锚未找到，拒绝静默成功")
        a=a[:m.end()]+block+a[m.end():]
    if a.count(start) != 1 or a.count(end) != 1:
        raise RuntimeError(f"注入后POOLLEDGER锚点异常: open={a.count(start)} close={a.count(end)}")
    J["bodies"]["auction"]=a
