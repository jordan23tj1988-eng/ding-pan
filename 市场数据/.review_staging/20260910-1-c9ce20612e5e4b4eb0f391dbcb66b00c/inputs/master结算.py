...
if __name__ == '__main__':
    # ★2026-09-10 防呆: 本脚本已自动注入 "master" 子命令, 命令行再写子命令名会被 argparse 当成 d 位置参数,
    #   报 "unrecognized arguments: YYYYMMDD"(EXIT=2, 20260909 复盘时实际踩坑)。此处置前置拦截并给出正解。
    _SUBS = {"audit", "coverage", "cognition", "master", "settle"}
    _pos = [a for a in sys.argv[1:] if not a.startswith("-")]
    if _pos and _pos[0] in _SUBS:
        sys.stderr.write(
            "用法: master结算.py YYYYMMDD --root INPUT_ROOT --out NEW_RUN_ROOT\n"
            "（子命令 master 由本脚本自动注入, 勿再手写 %s；"
            "P2 全量子命令请直接调: review_learning.py %s YYYYMMDD --root … --out …）\n"
            % (_pos[0], _pos[0]))
        raise SystemExit(2)
