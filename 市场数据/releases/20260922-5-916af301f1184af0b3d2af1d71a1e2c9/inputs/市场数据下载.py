...
def turnover_yi(date_str=None):
    """两市成交额(亿)=上证(全沪)+深证综指(全深),仅允许写入实时对应交易日。

    spot 接口返回当前时点数据；历史补跑严禁把当前成交额写进旧日期。
    历史成交额由市场温度.py 的独立历史指数源校准，取不到就保持 null。
    """
...
def main():
    ...
    summary = {"日期": d, "两市成交额_亿": turnover_yi(d), "涨停家数": n_zt, "跌停家数": n_dt,
