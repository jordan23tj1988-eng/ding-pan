    ...
    if IS_GOLDEN_HERO:
        if IS_INDEX:
            ...
            if route == 'limitup':
                ...
                try:
                    mt = _read_json(root / '_学习' / '_市场温度表.json').get(d, {})
                    ladder = mt.get('梯队') or {}
                    ladder_text = '、'.join(f'{str(k)}板{v}' for k, v in sorted(ladder.items(), key=lambda kv: int(str(kv[0]).replace('板',''))))
                    rm = _read_json(root / '_学习' / f'题材归位_{d}.json').get('档计数') or {}
                    ...
                    body += '<p class="hero-facts">梯队提炼：null；归位档：Anull、Bnull、Cnull。</p>'
