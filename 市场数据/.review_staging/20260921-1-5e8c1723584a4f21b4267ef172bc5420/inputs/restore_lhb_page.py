...
def build_page(d: str, current_html: str | None = None) -> str:
    ...
    # 前四段，避免把“席位分档库”漏掉导致黄金结构门禁误报。
    first_four = dynamic.find("<h2>四")
    if first_four < 0:
        raise RuntimeError("动态 body 缺少段四")
    dynamic_front = dynamic
    ...
    # 现有候选头部可能残留旧“五/六”标题，头部只允许公共组件，不允许业务 h2。
    head = re.sub(r'<h2[^>]*>.*?</h2>\s*', '', head, flags=re.S)
    # 第五、六项统一由标准能力进化模块承载；动态 body 中旧的
    # 自主深挖/认知迭代只用于审计，不再重复拼入展示页。
    # dynamic renderer 的业务前四段沿用黄金壳时，旧段尾部原本
    # 负责关闭一层包裹；移除旧段后显式闭合，保持 HTML 配平。
    body = head.rstrip() + "\n" + dynamic_front[h1:].lstrip()
    # 台账机器区内的历史“五/六”仅作审计记录，降级为 h3，避免与标准能力模块重复。
    body = re.sub(r'(?s)(<!--LHBLEDGER-->.*?<!--/LHBLEDGER-->)',
                  lambda m: re.sub(r'<h2>([五六])\s*', r'<h3>\1 ', m.group(1)), body)
    # 某些历史渲染器把台账锚点放在 h2 三段的机器块边界外；
    # 若前四段切片遗漏，补回原始成对锚点及其内容，不引入新数据。
    if body.count("<!--LHBLEDGER-->") != 1 or body.count("<!--/LHBLEDGER-->") != 1:
        body = re.sub(r"<!--LHBLEDGER-->.*?(?:<!--/LHBLEDGER-->|$)", "", body, flags=re.S)
        lm = re.search(r"<!--LHBLEDGER-->.*?<!--/LHBLEDGER-->", dynamic, re.S)
        if lm:
            body += "\n" + lm.group(0)
    ...
    # Deduplicate fifth/sixth headings when current head already contains them.
    _seen_h = set()
    def _dedup(m):
        title = m.group(1)
        if title[:1] in ("五", "六"):
    ...
    if page.count("<h2>") < 4: raise RuntimeError("龙虎榜基础段缺失")
