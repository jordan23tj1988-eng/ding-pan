...
def _sync_overview_evolution(src_index, route_files):
    ...
    # 主题页能力模块只能来自主题专用固定模板，禁止从 cycle/lhb/index 回灌。
    if any(target.name == 'theme.html' for target in route_files):
        template = Path(BASE) / '_学习' / '主题页能力进化模板.html'
        if not template.is_file():
            raise RuntimeError('主题页能力模板缺失，拒绝从其他页面回灌')
        src_index = template
...
def _postprocess_site(site_dir, date):
    ...
    if sync_targets and overview_ready:
        ...
        # 主题页不得从现站龙虎榜/周期页回灌；使用主题专用固定模板。
        theme_template = Path(BASE) / '_学习' / '主题页能力进化模板.html'
        if not theme_template.is_file():
            raise RuntimeError('主题页能力模板缺失，拒绝回灌: ' + str(theme_template))
        print('提示: 候选概览页无能力模块，使用主题专用能力模板:', theme_template)
        _sync_overview_evolution(theme_template, sync_targets)
