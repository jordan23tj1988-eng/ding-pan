...
def _theme_picks(root,d):
    """第三路(题材)荐票口径: 只读发出版 题材荐票_{d}.json, 与 module_render_theme.r_reco_table 同规则。
    新 schema=顶层'标的'数组(类型!='观察'计入荐票, '观察'不计); 旧 schema=荐票[]全量。
    返回 (picks, src); 发出版缺失=([], None) → 调用方不注入表格卡(零编造)。"""
    name='题材荐票_%s.json'%d
    path=root/'_学习'/name
    if not path.exists():return [],None
    ...
    picks=[x for x in base if str(x.get('类型') or '')!='观察']
    if not base:
        picks=[x for x in (data.get('荐票') or []) if isinstance(x,dict)]
    return picks,name

def _logic_picks(root,d):
    """第四路(产业逻辑)荐票口径: 只读发出版, 与 logic_pool.load_logic_picks 逐字同规则。
    新契约 logic判断_{d}.json: 荐票.标的[] 中 类型=='荐票'; 旧契约 逻辑荐票_{d}.json: 荐票[]全量。
    返回 (picks, src); 无发出版=([], None)。"""
    for name,strict in (('logic判断_%s.json'%d,True),('逻辑荐票_%s.json'%d,False)):
        ...
        picks=[x for x in items if isinstance(x,dict)
               and (not strict or str(x.get('类型','')).strip()=='荐票')]
        return picks,name
    return [],None
...
def _components(model,root):
    ...
    def generated_reco(key,section,mod,files,anchors=None):
        ...
        try:
            ...
            html=(module.r_reco_table(d) if mod=='theme'
                  else module.r_reco_table(d,body,picks,src))
            paths += [p for p in module._source_reads if p not in paths]
    ...
    if route=='index':
        ...
        # 荐票卡(锚 RECOTHEME): 既有 module_render_theme.r_reco_table 直渲 题材荐票_{d}.json;
        # 当日发出版空仓/缺失时不注入(由本栏目当日判断如实交代), 有票日才出表格卡。
        ...
        # 荐票卡(锚 RECOLOGIC): 既有 module_render_logic.r_reco_table 直渲 logic判断_{d}.json 的荐票;
        # 空仓日不注入(由本栏目当日判断如实交代), 有票日才出表格卡。
