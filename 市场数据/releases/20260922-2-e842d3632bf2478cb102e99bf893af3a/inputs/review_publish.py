...
from _review_contract import TOTAL_AUDIT_KEYS, ROUTE_AUDIT_KEYS, validate_schema_version

ROUTES = ("index", "cycle", "auction", "lhb", "theme", "logic", "limitup")
ANCHOR_PAIRS = {
    "auction": ("SCORECARD", "POOLLEDGER"),
    "lhb": ("FUNDTEMP", "LHBLEDGER"),
    "limitup": ("LEDGER",),
}

def anchor_errors(route, body):
    """机器锚必须成对且唯一；不再让缺闭锚静默丢整段。"""
    errors = []
    for token in ANCHOR_PAIRS.get(route, ()):
        op, cl = f"<!--{token}-->", f"<!--/{token}-->"
        n_open, n_close = body.count(op), body.count(cl)
        if n_open != n_close or n_open > 1:
            errors.append(f"{route}: {token}锚点不成对 open={n_open} close={n_close}")
    return errors
...
def exclusive(root, name=".review_publish.lock"):
    ...
    def stale():
        ...
        owner = {}
        try:
            owner = json.loads(meta.read_text(encoding="utf-8")) if meta.is_file() else {}
        except (OSError, ValueError, TypeError):
            owner = {}
        pid = owner.get("pid")
        # 有存活 owner 时绝不抢锁；无 owner 的旧锁只按 TTL 清理。
        return bool(pid is not None and not pid_alive(pid)) or (pid is None and age > stale_after)
...
def validate_inputs(frozen, d):
    ...
    try:
        ...
        audit_specs = [("总审", TOTAL_AUDIT_KEYS)] + [
            (route, ROUTE_AUDIT_KEYS) for route in ("auction", "lhb", "theme", "logic", "limitup")
        ]
        for label, allowed in audit_specs:
            ap = frozen / "_学习" / f"{label}_{d}.json"
            if not ap.exists():
                continue
            audit = read_json(ap)
            unknown = set(audit) - set(allowed)
            if unknown:
                errors.append(f"{ap.name} unknown fields: {sorted(unknown)}")
            schema_error = validate_schema_version(audit, ap.name)
            if schema_error:
        ...
        for route in ROUTES:
            ...
            if not isinstance(body, str) or not body.strip():
                ...
                errors.extend(anchor_errors(route, body))
...
def validate_pages(stage, result, d):
    ...
    for entry in pages:
        ...
        try:
            ...
            h.feed(raw)
            # Current p1.1-p1.3 store schema in the body attribute and date in kick.
            contract_id = str(result.get("contract") or "")
            version = str(expected_version or "")
            if re.fullmatch(r"p1\.[0-9]+", contract_id) and re.fullmatch(r"p1\.[0-9]+", version):
...
def validate_model_sources(root, d, models):
    ...
    for route, model in models.items():
        try:
            ...
            # 机器权威栏目(review_pages.MACHINE_ONLY_SECTIONS)由产出物直供, 不承载 LLM claim。
            optional_empty={('cycle','research'),('logic','forward'),('index','recommendations')}
...
def component_source_text(root,d,route,comp):
    ...
    key=comp['id']
    artifacts={'index':{'CROSSPICK':'跨路荐票卡','IDXLEAD':'先行指标灯','IDXVOTE':'周期投票牌'},
    ...
    if (route,key)==('logic','RECOLOGIC'):
        # 第四路荐票表: 独立复算 = 发出版(logic判断/逻辑荐票) → 与 logic_pool 同口径取荐票
        # → module_render_logic.r_reco_table。不读 presenter 产物; 空仓日 presenter 不产卡,
        # 若此处见到该组件即口径漂移, 直接判错(不静默放行)。
        with isolated_imports(root,root):
            for dep in ('logic_pool','_认知库渲染'):
                path=root/(dep+'.py')
                if path.exists():sys.modules[dep]=load_module(path)
            module=load_module(root/'module_render_logic.py')
            module.BASE=module.R=str(root);module.L=str(root/'_学习')
            def read_logic(name,date=None):
                ...
                return data
            module.load_json=read_logic
            body=(read_json(root/'_学习'/f'judgment_{d}.json') or {}).get('bodies',{}).get('logic') or ''
            if not body:raise ValueError('logic body missing')
            picks,src=sys.modules['logic_pool'].load_logic_picks(d,str(root/'_学习'))
            candidates,_candidate_src=sys.modules['logic_pool'].load_logic_candidates(d,str(root/'_学习'))
            if not src:raise ValueError('logic reco source missing')
            with readonly_files(root,root):
                return canonical_text(module.r_reco_table(d,body,picks,src,candidates))
    functions={'index':{'IDXTEMP':'r_mach_temp'},'cycle':{'VOLSTEP':'r_mach_volstep','LADDER':'r_mach_ladder'},
        'theme':{'THEMEBATTLE':'r_chart_matrix','LIFECYCLE':'r_chart_lifeaxis','RECOTHEME':'r_reco_table'},
