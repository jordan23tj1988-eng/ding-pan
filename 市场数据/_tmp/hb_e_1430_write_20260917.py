# -*- coding: utf-8 -*-
"""hb-e(14:30尾盘前哨) 20260917 catch_up 实例留档: 只写 ALARM_ONLY, 零 fills。
幂等/不覆盖: 目标文件已存在则报错退出(发出版不可覆盖)。"""
...
def tail_ticks():
    ...
    dn = sum(1 for x in rows if x["pct"] < 0)
    zt = sum(1 for x in rows if x["pct"] >= 9.8)
    dt = sum(1 for x in rows if x["pct"] <= -9.8)
    top = sorted(rows, key=lambda x: -x["pct"])[:4]
    bot = sorted(rows, key=lambda x: x["pct"])[:3]
    return {"path": "盘中/%s/realtime_ticks.jsonl" % TODAY, "n_tick": len(lines),
            "首条": first["ts"], "末条": last["ts"], "src": last.get("src"),
            "n": last.get("n"), "pool_date": last.get("pool_date"),
            "pool_stale": last.get("pool_stale"),
...
def idx_live():
    try:
        raw = subprocess.check_output(
            ["curl", "-s", "--max-time", "12", "https://qt.gtimg.cn/q=sh000001,sz399001,sz399006,sh000688"],
            timeout=20)
        ...
        for seg in txt.split(";"):
            ...
            if len(f) < 34:
                continue
            out.append("%s %s %+.2f%%" % (f[2], f[4], float(f[32])))
        return out
...
# ---- 断更分钟(相对名义决断时点 2026-09-16 14:30) ----
nominal = datetime.datetime(2026, 9, 16, 14, 30, 0)
freeze = datetime.datetime(2026, 9, 11, 10, 0, 11)   # launcher.log 冻结时点(9/11 hb-b/d 档留痕)
...
台账 = {"指数(我实测 %s, 腾讯码戳 12:0x)" % now.strftime("%H:%M"): indices,
        "观察池": ticks,
        "端口": ports}

breason = (
 "①【本场身份=过期场次 catch_up】executions.db: job_id=4deb15fee730 / execution_id=314f3f10d81c4b259c94a00d909cb3a4, "
 ...
 "lateness≈21.38h。即本实例名义场次=【9/16 尾盘前哨】, 而实际执行落在 9/17 午休段(11:5x)。"
 ...
 "③【9/17 现代场次未到点】本机当前 %s, 下午尚未开盘, 今日 14:30 名义段尚有约 2h24m 未到(jobs.json next_run_at=2026-09-17T14:30:00+08:00) → "
 "不得以『今日尾盘段』名义执行, 亦不预写 临盘决断_20260917_1430 之外的今日决断(写即把 11:5x 数据冒充 14:30 决断=后视镜+编造双违反)。" % write_ts +
 ...
 "⑥【C级禁止】最新总审=_学习/总审_20260911.json(档位C / 置信0.82 /『冰点防守…不形成进攻仓位』)→ 防守框架未解除, 且无覆盖 9/16、9/17 的总审 → 预案外进攻一律不做(铁律④退潮防守)。"
...
为备料 = {
 "尾盘卖预案候选": "无 —— 账本空仓(positions=[] / n_pos=0), 无持仓可卖 → 尾盘卖预案无对象(不是『未写』, 是『无对象』)。",
 "明日(9/18)预案要点草稿": "无合法生成路径 —— 明日预案的输入=本日收盘 fact/涨停池/龙虎榜/温度, 而日链三只计划任务实测『已禁用』"
   "(StockDailyChain 上次运行 2026/9/11 16:30、结果=1; 市场数据日目录 max=20260911) → 若收盘后日链仍不恢复, 9/18 预案同样断供。"
   "本场能交付的唯一『备料』=恢复清单(见收市后待批清单), 而非内容草案 —— 缺输入时编草案=违反铁律①。",
...
收市后 = ("按改动确认纪律, 盘中(及本场)零改动, 收市后先出方案+判断依据待批。顺序建议: "
 ...
 "③catch_up 派发机制: 每 job 只认领一个错失 instant → 9/15、9/17 两次『catch_up 风暴』(9/17 11:52:50–11:52:55 并行派发 9 个盘中 job) 复发, 建议改『按交易日逐段补跑』; "
 "④契约与实现对齐: prompt 指定的 pulse.json / 执行流水.jsonl 两条路径全盘 0 命中(本系统脉搏实为 warboard 内嵌 fact 字段) → 二选一(补实现 or 改契约); "
...
report = ("心跳hb-e无动作·仅报警: 本实例为 scheduler catch_up(名义 2026-09-16 14:30 尾盘前哨, 实际 9/17 11:52:54 执行, lateness≈21.4h) —— "
 "9/16 决断时点前零实时留档(通道自 9/11 10:00:11 冻结, 断更 %.1f 分钟)、9/17 14:30 现代场次尚未到点, 且预案(playbook max 9/07 / 计划 max 9/11)、"
 "持仓(空仓)、总审(9/11 档位C防守)三级全无动作对象 → ALARM_ONLY, 零 fills 零预判; 当日腾讯降级通道(sla 60s)虽新鲜但仅够台账、不够决断; "
...
dec = {
 ...
 "场次说明": ("14:30尾盘前哨心跳 hb-e, 本实例为 catch_up: scheduled_instant=2026-09-16T06:30:00+00:00(=2026-09-16 14:30 CST), "
   ...
   "今日(9/17)14:30 名义段未到点(jobs.json next_run_at=2026-09-17T14:30:00+08:00), 本文件不代其决断、不预写。"
   "结论: ALARM_ONLY, fills=[], 不写预判/触发区间/条件价。"),
 "条件决断": [],
 "data_freshness": {
   "交易日校验": {
     "任务指定": "python -c \"from sentiment.core.calendar import is_trading_day,today_str\" → ModuleNotFoundError: No module named 'sentiment'(本机无该包/未设PYTHONPATH) → 指定校验环节不可用。",
   ...
   "realtime_ticks": {"path": "盘中/%s/realtime_ticks.jsonl" % TODAY,
     "实测": "%s(首) → %s(末), 共 %d 条, 每 60s, src=%s, n=%s, pool_date=%s, pool_stale=%s" % (
        (ticks or {}).get("首条"), (ticks or {}).get("末条"), (ticks or {}).get("n_tick", 0),
        (ticks or {}).get("src"), (ticks or {}).get("n"), (ticks or {}).get("pool_date"), (ticks or {}).get("pool_stale")),
   ...
   "warboard": {"path": "盘中/%s/warboard.json" % TODAY, "exists": False,
     "fallback": "最新=盘中/20260909/warboard.json → 作战台自 9/10 起断供, 对 9/16、9/17 零指向(无持仓/预案/watch 状态可核)。"},
   ...
   "三账本": {"盘中作战": "本金=1000000 / nav=1000000.0 / 现金=1000000.0 / 持仓=[] / n_pos=0 / 起算=20260813(今日 11:53 零数据自评回写)",
     "口径": "盘中作战独立账本, 不共用 master 账; 空仓 100% 现金。"},
   "竞价快照": {"path": "_学习/竞价快照存档/%s_meta.json" % TODAY, "content": 竞价,
     "note": "本日 A 档竞价样本不存在: 采集器自标『失败 / em、sina 均不可达,不造数』(符合铁律①), 且采集时点 11:57:36 已在盘中 → 即便成功亦为污染样本。"},
   ...
   "stale_min": "相对名义决断时点(2026-09-16 14:30): 该时点前最后一条盘中留档=2026-09-11 10:00:11(launcher.log 冻结) → 断更 %.1f 分钟(≈124.5h), 远超 10 分钟红线 → 只报警禁决策; 相对当日(9/17)则通道新鲜(此反差本身即本期机制问题: 恢复时点晚于名义时点)。" % stale_min,
   "诊断证据(非决策输入)": {"指数": indices, "端口": ports, "说明": "均为 11:53–12:0x 运行态实测, 不构成市场判断或交易依据。"},
 },
 "decision": {"level": "ALARM_ONLY", "fills": [], "reason": breason,
   ...
   "note": "本场新增/复核要点: ①【正向】实时通道 11:52:55 重启恢复(腾讯降级源, tick 每 60s 续写), 结束自 9/11 起的零留档; 但池滞后(pool_stale=true)未解、无 Level1; "
     "②【负向】catch_up 风暴第 2 次复发(9/17 11:52:50–11:52:55 并行派发 hb-a/b/c/d/e+preread+deepb 等 9 个盘中 job), 同批 hb-b 于 12:04:19、deepb 于 11:58:40 已各自留档同型 ALARM_ONLY —— 三份留档结论一致、互不覆盖; "
     "③本文件与 报警_20260917.jsonl 并行留档(决策件+报警件), 均为只读追加语义, 不覆盖既有发出版。"},
 "收市后待批清单": 收市后,
 "后视镜边界声明": 后视镜,
 "report": report,
}
...
alarm = {
 "ts": write_ts, "date": TODAY, "session": "hb-e", "job_id": "4deb15fee730",
 "execution_id": "314f3f10d81c4b259c94a00d909cb3a4",
 "scheduled_instant": "2026-09-16T06:30:00+00:00 (= 2026-09-16 14:30 CST)",
 "actual_start": "2026-09-17T11:52:54+08:00",
 "kind": "catch_up",
 ...
 "reason": breason,
 "data_freshness": {
   "realtime_ticks": "盘中/%s/realtime_ticks.jsonl 实测 %s→%s 共 %d 条/每60s/src=%s/n=%s/pool=%s(stale=%s) → 当日新鲜(未触发当日红线), 但名义时点(9/16 14:30)前留档为零(断更 %.1f 分钟)" % (
     TODAY, (ticks or {}).get("首条"), (ticks or {}).get("末条"), (ticks or {}).get("n_tick", 0),
     (ticks or {}).get("src"), (ticks or {}).get("n"), (ticks or {}).get("pool_date"), (ticks or {}).get("pool_stale"), stale_min),
   "pulse": "盘中/%s/pulse.json exists=False(契约路径未实现, 全盘 0 命中) → 无脉冲对象可核" % TODAY,
   "warboard": "exists=False, 最新=20260909 → 对 9/16、9/17 零指向",
   "执行流水": "全盘不存在 → 无成交可对账",
   "预案真源": "playbook max=20260907; 交易计划 max=20260911(9/14–9/17 全缺, buys/sells 无可触发票据); 总审 max=20260911(档位C/防守, 置信0.82)",
   "三账本": "盘中作战: 本金100万 / 现金1000000 / 持仓=[] / n_pos=0 / 起算20260813 → 空仓",
 ...
 "半日实况(仅台账/非决策输入)": {"指数": indices, "观察池40只(9/11旧池)": ticks, "端口": ports,
   "说明": "11:53–12:0x 本人实测, 仅台账记录, 严禁作决策或训练输入。"},
 "机制发现": [
   "①catch_up 风暴第 2 次复发: 9/17 11:52:47–11:52:57 scheduler 并行派发 21 条今日执行(其中 9 个盘中 job: hb-a/b/c/d/e + preread + deepb + pipeline + watch), scheduled_instant 多为 9/15–9/16 各整点 → 『每 job 只认领一个错失 instant』缺陷复发(同 9/15 台账)。",
   ...
   "⑤交易日校验环节无可靠实现: sentiment.core.calendar 模块缺失 + 交易日历缓存末条=20260911, 两路皆不可判今日 → 建议补 9900(日历) 与 is_trading_day 接口。",
 ],
 "收市后待批清单": 收市后,
 "后视镜边界声明": 后视镜,
 "report": report,
...
print("OK alarm   :", alarm_path, os.path.getsize(alarm_path))
print("report:", report)
