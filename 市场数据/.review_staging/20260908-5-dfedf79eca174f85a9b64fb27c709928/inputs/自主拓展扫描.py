...
子命令:
 ...
 audit {d} --root INPUT_ROOT --out NEW_RUN_ROOT : P2全清单/推演/认知/Master审计，独立新run，不覆旧账本
...
def audit(d, *, root=None, out=None):
    """P2: 全清单归一+到期审计，新 run 写入显式 out，旧学习账本只读。"""
    from pathlib import Path
    from review_learning import audit_learning
    if root is None or out is None:
        raise ValueError("audit 必须显式传 root 与独立 out")
    return audit_learning(Path(root), d, Path(out))
