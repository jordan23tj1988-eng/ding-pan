# -*- coding: utf-8 -*-
"""hb-d 心跳 13:30午后延续段 2026-08-21: 只报警禁决策(数据全天断更)"""
...
# ---- 临盘决断留证(同8-17 hb-d先例: ALARM_ONLY, 无fills) ----
decision = {
    "date": TDAY,
    "session": "hb-d_1330",
    "ts": "13:30",
    ...
    "data_freshness": {
        ...
        "报警": {"path": f"盘中/报警_{TDAY}.jsonl",
                "note": "hb-a会话18:36:28已报同源data_stale(本心跳前已存在), 本心跳将去重追加hb-d条"},
        ...
        "stale_min": "全天(执行时刻18:37已过收盘, 盘中时段零留档, 远超10分钟红线)",
    ...
    "decision": {
        ...
        "reason": ("数据全天断更(同族第8日: 8-13~8-20+今日; 今日盘中实时管道未运行, 早段10:00:19全源失败报警退出后无恢复, "
                   ...
                   "③C级预案外进攻禁止(无数据无从核验, 写即编造违反铁律①零编造, 同hb-a/hb-b/hb-c今日先例); "
                   "依铁律只报警禁决策, 不写任何触发证据与预判"),
        "note": ("与今日hb-a(09:40开盘博弈段)同源报警; 取数链恢复并补pulse.json+当日warboard后, "
...
dpath = os.path.join(BASE, "盘中", TDAY, f"临盘决断_{TDAY}_1330.json")
with open(dpath, "w", encoding="utf-8") as f:
    json.dump(decision, f, ensure_ascii=False, indent=2)
print("DECISION_WRITTEN:", dpath)

# ---- 报警jsonl追加(去重 session=hb-d) ----
rec = {
    "ts": now,
    "session": "hb-d",
    "level": "ALARM",
    ...
    "detail": (
        "心跳hb-d(1330午后延续段)链路全天断更(同族第8日延续: 8-13/8-14/8-17/8-18/8-19/8-20/今日): "
        "盘中/20260821/ 仅playbook.json(08-20 19:42晚间预案), pulse.json/warboard.json/执行流水.jsonl/临盘决断 全MISSING; "
        "本心跳执行时刻=08-21 18:37(已过收盘, 盘中时段零留档), 13:30决断时间戳之前无任何当日行情数据可引; "
        ...
        "盘中/报警_20260821.jsonl 已有hb-a同源条(18:36:28, 本心跳前存在); "
    ...
    "disposition": (
        ...
        "③C级预案外进攻禁止(无数据无从核验, 写即编造违反铁律①零编造, 同今日hb-a先例); "
        "已写临盘决断_20260821_1330.json(ALARM_ONLY留证, 无fills); 无A/B/C级动作, 心跳hb-d无动作"
    ),
    "suggest": (
        ...
        "修复warboard引擎目录错位(8-20仍写盘中/20260820/正常, 8-21今日未生成); "
