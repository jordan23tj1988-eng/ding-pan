...
def _sync_overview_evolution(src_index, route_files):
    """Append the overview's two evolution sections to route pages — append-only, idempotent.

    2026-09-12 修复 #180 错位事故: 旧实现用 re.subn 整段替换本路 <section id="research">/<section id="cognition">,
    等于把每路自己的「六 自主深挖 / 七 我的认知迭代」删掉换成本模块。现在本函数只追加:
      1) 追加前断言本路 research/cognition 仍在位(缺=本路内容已被破坏 → raise, 不发布);
      2) 追加位置=页尾 foot 之前, 绝不替换本路板块;
      3) 模块序号从页面自身 section 数动态推出(本路末段+1/+2), 不写死, 防错位;
      4) 幂等: 重跑先清旧 sync 块, 不叠加。
    """
    import re
    overview = src_index.read_text(encoding='utf-8')
    sections = re.findall(r'<section class="evolution">.*?</section>', overview, re.S)
    if len(sections) != 2:
        raise RuntimeError('overview evolution sections != 2')
    css = ''
    # 只从 style 标签内部提取，不能在整份 HTML 上跨标签匹配；否则旧空 style
    # 的 id 文本会被拼进新 style，造成同名 DOM ID 重复。
    for style_body in re.findall(r'<style\b[^>]*>(.*?)</style\s*>', overview, re.S | re.I):
        css += ''.join(re.findall(
            r'[^{}]*\.(?:evolution|evo-stats|evo-stat|evo-list|evo-row|evo-empty)[^{}]*\{[^{}]*\}',
            style_body))
    if not css:
        raise RuntimeError('overview evolution CSS missing')
    style = '<style id="overview-evolution-sync">' + css + '</style>'
    CN = '一二三四五六七八九十'
    def _cn(n):
        return CN[n - 1] if 1 <= n <= len(CN) else str(n)
    for target in route_files:
        s = target.read_text(encoding='utf-8')
        if '<section id="research"' not in s or '<section id="cognition"' not in s:
            raise RuntimeError('route own modules missing before sync (refusing to overwrite): %s' % target)
        own_ids = [x for x in re.findall(r'<section id="([^"]+)"[^>]*>', s) if not x.startswith('evolution')]
        n_own = len(own_ids)
        if n_own < 5:
            raise RuntimeError('route section count implausible: %s (%d)' % (target, n_own))
        a, b = _cn(n_own + 1), _cn(n_own + 2)
        # 黄金源可能来自已经编号的周期/龙虎榜页，也可能来自概览裸标题；
        # 统一先去掉源标题序号，再按目标页自身段数编号，避免出现重复“五/六”。
        local_sections = [re.sub(r'<h2>[一二三四五六七八九十]?\s*自主拓展',
                                 '<h2>%s 自主拓展' % a, sections[0], count=1),
                          re.sub(r'<h2>[一二三四五六七八九十]?\s*认知迭代',
                                 '<h2>%s 认知迭代' % b, sections[1], count=1)]
        s = re.sub(r'\n*<!--OVERVIEW_EVOLUTION_SYNC_START-->.*?<!--OVERVIEW_EVOLUTION_SYNC_END-->\n*', '\n', s, flags=re.S)
        # 旧候选页可能已经留下空样式或重复样式；先清干净再只注入一份。
        s = re.sub(r'<style\b[^>]*\bid=["\']overview-evolution-sync["\'][^>]*>.*?</style\s*>',
                   '', s, flags=re.S | re.I)
        s = re.sub(r'<style\b[^>]*\bid=["\']overview-evolution-sync["\'][^>]*/>',
                   '', s, flags=re.S | re.I)
        s = s.replace('</head>', style + '</head>', 1)
        pos = s.rfind('<div class="foot">')
        if pos < 0:
            raise RuntimeError('page foot anchor missing: %s' % target)
        head = s[:pos].rstrip('\n')
        block = ('\n\n<!--OVERVIEW_EVOLUTION_SYNC_START-->\n' + local_sections[0] + '\n'
                 + local_sections[1] + '\n<!--OVERVIEW_EVOLUTION_SYNC_END-->\n')
        s = head + block + s[pos:]
        i_res = s.find('<section id="research"')
        i_evo = s.find('<!--OVERVIEW_EVOLUTION_SYNC_START-->')
        if not (0 <= i_res < i_evo):
            raise RuntimeError('evolution block misplaced (route module after sync block): %s' % target)
        if (s.count('<!--OVERVIEW_EVOLUTION_SYNC_START-->') != 1
                or s.count('<section class="evolution"') != 2
                or len(re.findall(r'<style\b[^>]*\bid=["\']overview-evolution-sync["\']', s,
                                  flags=re.I)) != 1):
            raise RuntimeError('evolution block duplicated: %s' % target)
        target.write_text(s, encoding='utf-8', newline='\n')


def _postprocess_site(site_dir, date):
    """页面发布统一后处理(2026-09-12)：概览能力模块同步 + cycle/lhb 黄金视觉锁。

    同一函数既跑候选站点(release stage/site，门禁检查前)，也跑现站到站兜底，
    从而保证“被门禁检查的页面”与“被部署的页面”同源、同字节。
    幂等：已同步/已黄金化的页面自动跳过对应步骤(按 section#research/cognition 与恢复器断言判定)。
    """
    import os as _os
    ...
    def _has_own_modules(p):
        s = p.read_text(encoding='utf-8')
        return '<section id="research"' in s and '<section id="cognition"' in s
    # 通用能力模块同步：仍带 section#research/cognition 的路(index/cycle 走各自专属链)。
    # lhb 在候选页阶段仍在名单内，同步完再由恢复器接管为黄金版 h2 直连。
    # 本轮用户范围只恢复主题页；auction/logic/limitup 仍按各自现有契约，
    # 不因主题页补齐能力模块而被连带改版。
    sync_targets = [site / 'theme.html'] if _has_own_modules(site / 'theme.html') else []
    import re as _re
    live_site = Path(SITE)
    ...
    if sync_targets and overview_ready:
        _sync_overview_evolution(overview, sync_targets)
    elif sync_targets and not overview_ready:
        # 候选概览页能力模块在到站阶段才重建；但主题/逻辑等页面不能因此漏掉
        # 与周期/龙虎榜完全同构的“自主拓展/认知迭代”模块。回退到现站龙虎榜
        # 黄金页取同一版式与能力台账，追加而非替换各路自己的 research/cognition。
        fallback_evolution = live_site / 'lhb.html'
        if not fallback_evolution.is_file():
            raise RuntimeError('能力模块回退源缺失: ' + str(fallback_evolution))
        print('提示: 候选概览页无能力模块，改用现站龙虎榜黄金模块源:', fallback_evolution)
        _sync_overview_evolution(fallback_evolution, sync_targets)
    elif not overview_ready:
        # cycle/lhb 自身的黄金恢复仍使用现站能力模块/CSS 源；无可同步页面时只输出提示。
        print('提示: 候选概览页无能力模块且无自有模块页面，使用现站周期/龙虎榜源', live_site)
    if not overview_ready:
        # cycle 能力模块与其样式均沿用现站周期页(六/七 编号，黄金版口径)。
        # 概览页不内嵌 evolution CSS，不能把 index.html 当 CSS 回退源。
        env['CYCLE_EVO_SOURCE'] = str(live_site / 'cycle.html')
        env['CYCLE_CSS_SOURCE'] = str(live_site / 'cycle.html')
    ...
    for repair in (Path(BASE) / 'restore_cycle_page.py', Path(BASE) / 'restore_evolution_style.py'):
        ...
        cp = _sp.run([sys.executable, str(repair), date], cwd=BASE, capture_output=True, text=True,
    ...
    cp = _sp.run([sys.executable, str(lhb_repair), date], cwd=BASE, capture_output=True, text=True,
                 encoding='utf-8',
    ...
    if cp.returncode != 0:
        raise RuntimeError('lhb visual repair failed: ' + str(lhb_repair) + '\n'
                           + cp.stdout[-2000:] + cp.stderr[-2000:])
    synced = ('+'.join(p.name for p in sync_targets) if sync_targets else '(无)') \
        if (sync_targets and overview_ready) else '(跳过: 候选概览页无能力模块，改用现站源)'
    print('统一后处理完成:', site, '| 能力模块同步', synced, '| cycle/lhb 黄金视觉锁')
...
def _deploy_site(date, result):
    ...
    if not rel:
        return  # 无发布产物(测试fixture/未发布)时无到站
    rel = Path(rel)
    src = rel / 'site'
    if not src.is_dir():
        raise RuntimeError('release site missing: ' + str(src))
    ...
    # 统一后处理(2026-09-12)：能力模块同步 + cycle/lhb 黄金视觉锁。
    # 候选发布已在门禁前跑过同一套(review_publish._postprocess_stage)，此时候选页即终版，
    # 复制过来就是“被门禁检查的页面”；未跑过(旧调用方/手工恢复)时到站兜底补跑，同函数同字节。
    if (result.get('stage_postprocess') or {}).get('ok'):
        print('到站: 候选站点已完成统一后处理，跳过二次处理(与门禁检查同源)')
    else:
        _postprocess_site(Path(SITE), date)
    ...
    if os.path.exists(jpath):
        ...
        upd = j.get('更新label', date); tick = j.get('ticker', '')
        ...
        if ab8:
            ...
            with open(os.path.join(ARC, date + '.html'), 'w', encoding='utf-8', newline='\n') as f:
                f.write(shell('复盘存档 ' + date, topbar, _rt(ab8), tick))
        ...
        dates = sorted([os.path.splitext(os.path.basename(x))[0] for x in glob.glob(os.path.join(ARC, '*.html'))], reverse=True)
        lis = ''
        for dt in dates:
            disp = dt[4:6] + '-' + dt[6:8]
        ...
        with open(os.path.join(SITE, 'history.html'), 'w', encoding='utf-8', newline='\n') as f:
            f.write(shell('情绪盯盘台 · 历史', nav('history.html', upd), hb, tick))
    print('到站完成:', date, '| 7主页面复制 + 存档/历史更新')


def _run_p2_production_audit(date):
    """Run P2 against explicit production input and independent audit output."""
    import subprocess
    audit_root = os.path.join(os.path.dirname(BASE), "_复盘审计运行")
    cmd = [sys.executable, '-B', os.path.join(BASE, 'p2_production_audit.py'),
           date, '--root', BASE, '--out', audit_root]
    p = subprocess.run(cmd, cwd=BASE, capture_output=True, text=True,
                       encoding='utf-8', errors='replace', timeout=900,
                       env={k: v for k, v in os.environ.items() if k != 'PYTHONPATH'})
    print('[P2 production audit]', p.stdout.strip()[-2000:])
    if p.returncode != 0 and p.stderr:
        print('[P2 production audit stderr]', p.stderr.strip()[-2000:])
    return p.returncode == 0


def _run_p3_production_bridge(date):
    """Run P3 qualification bridge; no production transaction is created."""
    import subprocess
    out = os.path.join(os.path.dirname(BASE), "_复盘审计运行", "p3", date + '.json')
    cmd = [sys.executable, '-B', os.path.join(BASE, 'p3_production_bridge.py'), date,
           '--root', BASE, '--out', out]
    p = subprocess.run(cmd, cwd=BASE, capture_output=True, text=True,
                       encoding='utf-8', errors='replace', timeout=120,
                       env={k: v for k, v in os.environ.items() if k != 'PYTHONPATH'})
    print('[P3 production bridge]', p.stdout.strip()[-2000:])
    if p.returncode != 0 and p.stderr:
        print('[P3 production bridge stderr]', p.stderr.strip()[-2000:])
    return p.returncode == 0


def build(date):
    """Compatibility command: all writes go through the verified release gate.

    No legacy fallback: a failed/degraded candidate must not touch CURRENT.
    The old shell/nav helpers above remain reusable by the deterministic renderer.
    PASS 后到站(2026-09-07): releases/<id>/site 7主页面 -> 复盘/盯盘台 + 当日存档/历史重建。
    """
    from pathlib import Path
