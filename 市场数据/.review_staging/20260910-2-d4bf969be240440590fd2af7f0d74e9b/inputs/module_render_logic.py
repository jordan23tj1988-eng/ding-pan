...
from logic_pool import load_logic_picks, load_logic_candidates
    ...
    for x in top:
        ...
        obs += ('<div class="obs"><div class="obs-head"><span class="obs-nm">%s <span class="mut">%s</span></span>'
                ...
                % (_esc(nm), _esc(code), _esc(str(imp)),
                   ...
                   _esc(str(lines)), _esc(str(words)), _esc(str(reason))))
    return '<!--MACHRADAR-->\n%s%s<!--/MACHRADAR-->\n' % (fld, obs)

# ============ 机器区: 荐票历史结算 ============
def r_mach_hist(d=None):
...
def r_reco_table(d, body, picks, src, candidates=None):
    """板块一荐票表：正式荐票与发出版观察候选共用一张表。
    空仓仍保留表头；观察候选只作相对价值观察，不进入正式荐票/结算；缺源另行断档。"""
    candidates = [x for x in (candidates or []) if isinstance(x, dict)]
    if picks or candidates:
        rows = []
        def add_row(i, x, is_rec):
            nm = _esc(str(x.get('名称', '')))
            code = _esc(str(x.get('代码', '')))
            typ = _esc(str(x.get('类型') or ('荐票' if is_rec else '观察')))
            ...
            rows.append(('<tr><td>%d</td>'
                        ...
                        % (i, nm, code,
                           ('<b>%s</b>' % typ) if is_rec else ('<span class="mut">%s</span>' % typ),
                           ('[%s] ' % chain) if chain else '',
                           ('·%s ' % link) if link else '', reason, hist))
        for i, x in enumerate(picks, 1):
            add_row(i, x, True)
        for i, x in enumerate(candidates, len(picks) + 1):
            add_row(i, x, False)
        ...
        if picks:
            lead = '正式荐票来自本路发出版；观察候选不进入正式荐票/结算。'
        else:
            lead = '本路仓位结论：空仓；以下为发出版保留的相对价值观察候选，不进入正式荐票/结算。'
        hint = ('<div class="hint">发出版不可覆盖(%s(发出版)); %s 荐票=兑现逻辑可结算化≠确认; 结算=兑现口径</div>'
                % (src, lead))
        return table + chr(10) + hint
    if src:
        table = ('<div class="card"><table style="table-layout:fixed;width:100%%"><colgroup>'
                 '<col style="width:24px"><col style="width:104px"><col style="width:80px"><col><col style="width:170px"></colgroup>'
                 '<tr><th>#</th><th>标的</th><th>类型</th><th>逻辑(链条·环节·理由)</th><th>同类历史</th></tr></table></div>')
        return table + ('<div class="hint">发出版不可覆盖(%s(发出版)); 本路仓位结论：空仓；本日发出版没有合法候选，不补造标的。</div>' % src)
    # src 无 → 旧日期 body obs 兜底，仅在确实没有发出版时明确标注断档。
    rows = []
    obs_rows = []
    ...
    if not rows:
        return ('<div class="card" style="border-color:rgba(255,95,86,.35)"><b>⚠ 荐票发出版缺失</b>'
                '<p style="margin:6px 0 0">发出版文件不存在且body无可回溯荐票候选；这是断档，不是空仓。—</p></div>')
    hint = ('<div class="hint">⚠ 发出版缺失(当晚未落盘)——以下表格由agent手写obs卡照转，'
            '仅作待补产证据，不代表正式荐票；结算口径不变</div>')
    table = ('<div class="card"><table style="table-layout:fixed;width:100%%"><colgroup>'
             '<col style="width:24px"><col style="width:104px"><col style="width:80px"><col><col style="width:170px"></colgroup>'
             '<tr><th>#</th><th>标的</th><th>类型</th><th>逻辑(链条·环节·理由)</th><th>同类历史</th></tr>%s</table></div>' % ''.join(rows))
    return table + chr(10) + hint
...
def build_page_full(d, paper_block=''):
    ...
    if body:
        ...
        _candidates, _candidate_src = load_logic_candidates(d, L)
        ...
        if i1 >= 0:
            ...
            out = head + '\n' + r_reco_table(d, body, _picks, _src, _candidates) + r_insight(body) + seg2 + (out[i2:] if i2 > 0 else '')
