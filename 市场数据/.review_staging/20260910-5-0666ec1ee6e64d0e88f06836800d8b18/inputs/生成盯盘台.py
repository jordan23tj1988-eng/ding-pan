...
def _sync_overview_evolution(src_index, route_files):
    """同步两块标准能力进化模块到主题页，幂等且不保留旧重复模块。

    2026-09-13 用户确认：主题页删除重复的旧 research/cognition 展示层，
    只保留标准 evolution 两块。函数仍保护其他业务 section，并且：
      1) 允许 theme.html 不再有 research/cognition；其他路缺失仍拒绝发布；
      2) 追加位置=页尾 foot 之前；
      3) 模块序号从剩余业务 section 数动态推出；
    ...
    for target in route_files:
        ...
        is_theme = target.name == 'theme.html'
        if (not is_theme) and ('<section id="research"' not in s or '<section id="cognition"' not in s):
        ...
        if is_theme:
            placement_ok = i_res < 0
        else:
            placement_ok = 0 <= i_res < i_evo
        if not placement_ok:
...
def _postprocess_site(site_dir, date):
    ...
    # 本轮只处理主题页：渲染器已省略旧 research/cognition，
    # 这里只追加一组标准 evolution；auction/logic/limitup 不连带改版。
    sync_targets = [site / 'theme.html'] if (site / 'theme.html').is_file() else []
    import re as _re
    live_site = Path(SITE)
