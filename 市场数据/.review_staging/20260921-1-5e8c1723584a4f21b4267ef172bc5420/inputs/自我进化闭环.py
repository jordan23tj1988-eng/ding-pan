...
def build(d):
    ...
    for p in LEARN.glob('_认知库_*.json'):
        ...
        for i,x in enumerate(data.get('条目',[]) or []):
            ...
            old.update({'id':ident,'fingerprint':ident,'kind':'认知迭代','route':route,'date':x.get('日期'),'title':x.get('标题'),'body':x.get('正文'),'falsifiable_rule':x.get('可证伪条件'),'state':state,'validation':old.get('validation'),'hit_count':old.get('hit_count'),'capitalized':old.get('capitalized'),'reused_count':old.get('reused_count'),'review_log':old.get('review_log') or x.get('复核日志') or [],'last_reviewed_at':old.get('last_reviewed_at'),'feedback_routes':old.get('feedback_routes') or [],'source':x.get('来源') or p.name})
