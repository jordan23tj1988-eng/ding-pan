...
HEADING_MAP = {
 'index': [('总判断','observations'),('核心观察','observations'),('五路','routes'),('拐点','turning'),('检查四项','turning'),('总裁决','verdict'),('Master','master'),('深挖','master'),('认知','master'),('指派','master')],
 'cycle': [('量能','volume'),('先行','leading'),('五阶段','stages'),('周期投票','stages'),('连板梯队','ladder'),('攻防','position'),('深挖','research'),('认知','cognition')],
 'auction': [('当日','pool'),('昨日池','settlement'),('温度','temperature'),('胜率','winrate'),('深挖','research'),('认知','cognition')],
 ...
 'logic': [('承接Master指派','research'),('产业逻辑·业绩腿命门','recommendations'),('产业逻辑判断','hardness'),('荐票','recommendations'),('链条','chains'),('产业链','chains'),('逻辑硬度','hardness'),('前置','forward'),('风险日历','forward'),('中报','earnings'),('深挖','research'),('认知','cognition')],
...
def _body_import(model, body, filename):
    ...
    def visit(node):
        ...
        if atomic and not node.has('h2'):
            ...
            role = 'cognition' if section=='cognition' else 'research' if section=='research' else 'observation' if 'obs' in classes else 'evidence'
...
def _legacy(model, root):
    ...
    if data:
        allowed = ('日期','date','路','来源','结论','五路裁决','总裁决','检查四项','分歧裁决','综合深挖','认知迭代','线索跟踪','指派清单') if route=='index' else ('日期','date','路','来源','判断','荐票','认知迭代','认知迭代_条目','深挖')
        _keys(data,allowed,filename)
        if '路' in data and data['路']!=route:
            raise ValueError(filename+' 路不匹配')
        for top,value in data.items():
            if top in ('日期','date','路','来源'):
                continue
            if route=='index':
                section = {'结论':'verdict','五路裁决':'routes','总裁决':'verdict','检查四项':'turning','分歧裁决':'turning','综合深挖':'master','认知迭代':'master','线索跟踪':'master','指派清单':'master'}[top]
                role = 'cognition' if top=='认知迭代' else 'research' if top=='综合深挖' else 'counterevidence' if top=='分歧裁决' else 'verdict' if top in ('结论','总裁决') else 'evidence'
            else:
    ...
    model['complete'] = bool(data or body) and all(s['claim_refs'] for s in model['sections'])
    if data or body:
...
def _components(model,root):
    ...
    model['components']=[]
    def record(key,section,producer,paths,html,anchors=None,issue=None):
        ...
        comp={'id':key,'section':section,'producer':producer,'status':'ok' if valid else 'missing',
              ...
              'issue':issue or (None if valid else '当日权威机器源缺失')}
    ...
    def ledger(anchor,section):
        ...
        comp=record(anchor,section,'归档台账脚本注入的冻结 body 锚区',[p],match[1] if match else None)
        if comp['html']:
    ...
    if route=='index':
        ...
        try:
            ...
            cycle_body = ''
        has_cycle_body = bool(cycle_body and '<h2>一' in cycle_body)
        ...
        generated('MACHRADAR','earnings','logic','r_mach_radar',['中报预增雷达_'+d+'.json'])
    ...
    missing=[c['id'] for c in model['components'] if c['status']!='ok' or c.get('missing_values')]
...
def _render(model,contract):
    ...
    if hero_id:
        shown.add(hero_id)
    body = model.get('ticker','') + '<div class="rowA"><div class="hero"><div class="kick">' + e(model["title"]) + ' · ' + d + ' · ' + e(model["status"]) + '</div><h1' + (' id="claim-' + e(hero_id) + '"' if hero_id else '') + '>' + e(model["hero"]["text"]) + '</h1>'
