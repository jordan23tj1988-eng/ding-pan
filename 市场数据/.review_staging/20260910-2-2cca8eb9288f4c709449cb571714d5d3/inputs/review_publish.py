...
def scoped_p12_check(root,stage,d,route,*,raw=None):
    ...
    try:
        ...
        if golden_route:
            ...
            # 黄金壳的 component-* id/section 壳被恢复器剥离；每个模型组件仍须
            # 有唯一成对生产锚点，且锚点内容必须与冻结生产组件一致。
            for comp in model.get('components', []):
                ...
                if raw.count(start) != 1 or raw.count(end) != 1:
                    # SEATLIB 是黄金版第四段的既有展示内容，不使用独立机器锚；
                    # 其来源仍由 lhb 数据核对器校验，不能因壳恢复强行重造重复组件。
                    if not (route == 'lhb' and key == 'SEATLIB'):
                        errors.append('golden component anchor pair ' + str(key))
                    continue
                i = raw.find(start) + len(start)
                j = raw.find(end, i)
                actual = canonical_text(raw[i:j])
                expected = canonical_text(comp.get('html', ''))
                if actual != expected:
                    errors.append('golden component content mismatch ' + str(key))
            return {
...
def _postprocess_stage(root, stage, d):
    ...
    if not script.is_file():
        return {"ok": False, "error": "生成盯盘台.py missing"}
    ...
    return {"ok": True, "stdout": cp.stdout[-800:]}
...
def view_check(root,stage,d,route):
    ...
    try:
        ...
        for claim in model['claims']:
            ...
            # 主题页按展示治理隐藏重复的旧版 bodies 摘录和荐票观察包装；
            # 它们仍在 model/audit 中，不能再被 P1 误判为数据丢失。
        ...
        try:
            ...
            for width in (1440,390):
                ...
                for route in ROUTES:
                    ...
                    if(row){const items=[...row.children].filter(x=>x.matches('.hero,.kpi'));const compactCycle=document.body.dataset.route==='cycle'&&items.length===2&&items[0].matches('.hero')&&items[1].matches('.kpi')&&items[1].children.length===4;if(!compactCycle&&(items.length!==5||!items[0].matches('.hero')||items.slice(1).some(x=>!x.matches('.kpi'))))errors.push('rowA hero/four KPI contract');}
