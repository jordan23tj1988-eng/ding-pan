...
def cog_text(route, applied):
    text, as_of, n = cog_latest(route)
    return ('认知库截至%s（%d条）作为决策前认知；本日%s。认知库无当日晚间新增条目，未伪造当日认知。最近条目：%s'
            % (as_of, len(COG[route]['条目']), applied, text[:420]))
...
P['index'] = build('index', [
    ...
    ('master', 'cognition', cog_text('master', '总审按防守原则处理五路分歧，未升级席位路局部信号'), ev('_学习/_认知库_master.json', '/条目/0/正文')),
])
P['cycle'] = build('cycle', [
    ...
    ('cognition', 'cognition', cog_text('cycle', '周期按中性温度处理，未升级为进攻'), ev('_学习/_认知库_cycle.json', '/条目/0/正文')),
])
P['auction'] = build('auction', [
    ...
    ('cognition', 'cognition', cog_text('auction', '缺有效9:25轨迹时维持C档，不以静态结构替代动态确认'), ev('_学习/_认知库_auction.json', '/条目/0/正文')),
])
P['lhb'] = build('lhb', [
    ...
    ('cognition', 'cognition', cog_text('lhb', '局部强席位信号经双源、样本和总环境三重约束后仅保留条件性轻仓'), ev('_学习/_认知库_lhb.json', '/条目/0/正文')),
])
theme_specs = [
    ...
    ('cognition', 'cognition', cog_text('theme', '题材宽度不替代催化与跨环节确认，维持C档'), ev('_学习/_认知库_theme.json', '/条目/0/正文')),
]
theme_matrix_claim = {
...
P['logic'] = build('logic', [
    ...
    ('cognition', 'cognition', cog_text('logic', '预增A成色池必须与涨停池及题材承载同步验证，缺失时禁止误读成零A档'), ev('_学习/_认知库_logic.json', '/条目/0/正文')),
])
P['limitup'] = build('limitup', [
    ...
    ('cognition', 'cognition', cog_text('limitup', '结构、封板质量与主线催化分开，缺一不升档'), ev('_学习/_认知库_limitup.json', '/条目/0/正文')),
])
