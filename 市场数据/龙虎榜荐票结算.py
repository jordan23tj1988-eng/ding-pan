...
def main(dprev):
    ...
    refl=(f"{dprev}席位路荐票T+1结算:执行胜率{win}/{n},Top5均收{top_avg}% vs 全场上榜均收{mkt_avg}%"
    ...
    json.dump(dict(荐票日=dprev,Top5=res,汇总=dict(执行胜率=f"{win}/{n}",执行均收=top_avg,全场均收=mkt_avg,选股增益pp=edge),
        ...
        open(os.path.join(L,f"龙虎榜荐票结算_{dprev}.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1)
    ...
    blk=(f'<p style="font-weight:700;margin:4px 0 4px;border-left:3px solid var(--accent);padding-left:8px">当日席位Top5荐票 · T+1结算对账(执行口径)</p>'
     f'<div class="hint">汇总:执行胜率{win}/{n} · Top5均收{top_avg}% vs 全场上榜{mkt_avg}%'
    ...
    print(f"{disp} 席位Top5结算: 胜率{win}/{n} Top5均收{top_avg}% vs 全场{mkt_avg}% 增益{edge}pp | {inj}")
    print("反思:",refl)
