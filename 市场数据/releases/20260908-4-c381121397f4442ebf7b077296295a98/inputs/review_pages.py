...
def _render(model,contract):
    ...
    return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>' + e(model["title"]) + '</title><style>' + contract["visual"]["css"] + '</style></head><body data-route="' + e(route) + '">' + nav + '<div class="wrap">' + body + contract["visual"]["foot"] + '</div>' + VISUAL_JS + '</body></html>\n'
