...
# 概览页黄金骨架锁(2026-09-23 用户拍板): 段序/组件形态冻结, 每次复盘不得漂移。
# 段序=一 明日核心观察点(.obs) 二 拐点预警 三 五路看牌 四 总裁决; 五/六 能力进化模块由部署后处理注入。
GOLDEN_INDEX_ROUTES = ("index",)
# 黄金四段: 契约只有声明这四段(且顺序一致)才启用黄金骨架锁。
# 冻结集成根/历史契约(observations/master 等)仍走既有渲染路径, 避免旧根被锁误伤。
GOLDEN_INDEX_SECTIONS = ("recommendations", "turning", "routes", "verdict")
...
def _page_css(contract, route=None):
    ...
    if route not in V44_HIDDEN_ROUTES:
        if route == 'index':
            # 概览黄金骨架例外(2026-09-23 用户指正「图2没有对应的股票名称」):
            # 黄金版 .obs 卡头部就是「名称+代码」与「身位」标签, 这条去噪规则会把票名吃掉。
            # 只对概览去掉该条, 其余去噪规则与其余页面口径不变。
            base += V44_CSS.replace('.obs-head .obs-nm,.obs-watch>.obs-lab{display:none}', '')
        else:
            base += V44_CSS
    if route == 'index':
        # 黄金概览已撤阅读地图与机器折叠层；连同其遗留样式选择器一并
        # 从结果页 CSS 移除，避免已下线的 page-map/machfold 契约残留。
        base = base.replace('page-map', 'golden-legacy-map').replace('machfold', 'golden-legacy-fold')
    return base
...
ANCHORS = {
 'index': {'CROSSPICK':'recommendations','IDXTEMP':'turning','IDXLEAD':'turning','IDXVOTE':'turning'},
...
HEADING_MAP = {
 'index': [('总判断','verdict'),('核心观察','turning'),('环境','turning'),('题材树','routes'),('每路独立核算','routes'),('周期与攻防','turning'),('五路看牌','routes'),('五路','routes'),('明日观察','turning'),('拐点预警','turning'),('拐点','turning'),('检查四项','turning'),('发布门禁','turning'),('推演指派','verdict'),('总裁决','verdict'),('Master','verdict'),('深挖','verdict'),('认知','verdict'),('指派','verdict')],
...
def _legacy(model, root):
    ...
    if data:
        allowed = TOTAL_AUDIT_KEYS if route=='index' else ROUTE_AUDIT_KEYS
        _keys(data,allowed,filename)
        schema_error = validate_schema_version(data, filename)
        if schema_error:
        ...
        for top,value in data.items():
            if top in ('日期','date','路','来源','schema_version','昨日战绩验收','环境加权依据','页面合同'):
                continue
            if route=='index':
                section = {'结论':'verdict','五路裁决':'routes','总裁决':'verdict','检查四项':'turning','发布门禁':'turning','推演指派':'verdict','分歧裁决':'turning','分歧点':'turning','综合深挖':'verdict','认知迭代':'verdict','线索跟踪':'verdict','指派清单':'verdict'}[top]
...
def _components(model,root):
    ...
    if route=='index':
        ...
        if d in table:generated('IDXTEMP','turning','index','r_mach_temp',['_市场温度表.json'])
        else:record('IDXTEMP','turning','module_render_index.r_mach_temp',[],None)
        ...
        artifact('IDXVOTE','turning','周期投票牌_'+d+'.html','周期投票.py')
...
def _render(model,contract,root=None):
    ...
    # 概览黄金骨架开关(2026-09-23): 由契约段序决定, 而非仅仅 route==index。
    ...
    import module_golden_index as _golden_index
    claims = {c["id"]: c for c in model["claims"]}
    ...
    if GOLDEN_INDEX_ON and root:
        ...
        _zj = _dated(root, 'judgment_' + d + '.json', d)
        _hl = str((_zj or {}).get('一句话') or '').strip()
        if not _hl:
            _hl = str((_dated(root, '总审_' + d + '.json', d) or {}).get('一句话') or '').strip()
        if _hl:
            hero_title = _hl
    ...
    if IS_GOLDEN_HERO:
        if IS_INDEX:
            ...
            pills += '<span class="pill warn">情绪档 · <b class="s-weak">' + e((model["kpis"][0].get("chip") or '—').split(' ')[0]) + ' · 温度' + e(temp) + '</b></span>'
    ...
    if IS_INDEX:
        for comp in model.get('components', []):
            if comp['id'] not in ('IDXTEMP', 'IDXVOTE') or comp['status'] != 'ok':
                continue
    ...
    if mach_parts and not GOLDEN_INDEX_ON:
        ...
        # 组件读数仍在 model/components 与模型 JSON 里(供门禁与页内锚点复核)。
    ...
    golden_bodies = {}
    if GOLDEN_INDEX_ON:
        # 概览页黄金骨架锁(2026-09-23 用户拍板): 四段段序与组件形态由
        # module_golden_index 唯一决定; 总审正文/claim 不得改变段落增删、顺序与卡片形态。
        import module_golden_index as _golden_index
        ...
        if 'turning' in _absorbed and 'recommendations' in golden_bodies:
            # turning 的真实卡仍在段一 rail；段标记改由下面的 hidden 结构段承接，
            # 避免同一 GOLDEN-INDEX 标记出现两次，同时不改变 rowC/rail 版式。
            golden_bodies['recommendations'] = golden_bodies['recommendations'].replace(
                '<!--GOLDEN-INDEX:turning-->', '').replace(
                '<!--/GOLDEN-INDEX:turning-->', '')
        _want = [s['id'] for s in render_sections if s['id'] not in _absorbed]
        _miss = [k for k in _want if k not in golden_bodies
                 or '<!--GOLDEN-INDEX:%s-->' % k not in golden_bodies[k]]
        if _miss:
            # 契约不符即 fail: 宁可不写候选页, 也不让骨架漂移的页面发出去。
            raise ValueError('概览黄金骨架契约不符: 缺段/缺标记 %s' % _miss)
    for number, s in zip('一二三四五六七', render_sections):
        if GOLDEN_INDEX_ON and s['id'] in _absorbed:
            if s['id'] == 'turning':
                # turning 的内容已在段一 .rowC/rail 展示；保留不可见结构段、h2
                # 与黄金标记，供结构门禁识别，但不制造第二个可见大段。
                body += ('<section id="turning" hidden aria-hidden="true"><h2>'
                         + number + ' ' + e(s["title"]) + '</h2>'
                         + '<!--GOLDEN-INDEX:turning--><!--/GOLDEN-INDEX:turning--></section>')
            continue  # 已并入段一右栏(黄金版 .rowC 版式), 标题随卡在右栏内
        body += '<section id="' + e(s["id"]) + '"><h2>' + number + ' ' + e(s["title"]) + '</h2>'
        inner = ''
        if s['id'] in golden_bodies:
            body += golden_bodies[s['id']] + '</section>'
            continue
        for comp in model.get('components',[]):
        ...
        # 概览页黄金锁(2026-09-23): 结果层只留黄金组件, 判断卡/旧版正文/门禁折页
        # 不再上页; claim 原文与证据回链由页尾无痕原文库+来源审计承接, 数据一条不删。
        if GOLDEN_INDEX_ON:
            visible_claim_refs = []
        elif route == 'theme' and s['id'] in ('matrix', 'lifecycle'):
            visible_claim_refs = []
        else:
            visible_claim_refs = s['claim_refs']
    ...
    if route not in AUDIT_FOLD_HIDDEN_ROUTES and not GOLDEN_INDEX_ON:
        ...
        # 不可见证据锚点(audit-anchor-bank)留在页内供门禁复核。
    ...
    # 黄金概览不恢复机器数据核对层，只补齐机器组件的内部成对锚点。
    # 已由黄金可见组件承接的锚点不重复写入；其余锚点放入 hidden 容器，
    # 供模型/发布门禁核对，避免结果层重新出现机器读数或审计折叠。
    hidden_machine_anchors = ''
    if GOLDEN_INDEX_ON:
        missing_machine_anchors = []
        for anchor in model.get('machine_anchors', {}):
            start = '<!--' + e(anchor) + '-->'
            end = '<!--/' + e(anchor) + '-->'
            start_count, end_count = body.count(start), body.count(end)
            if start_count == 0 and end_count == 0:
                missing_machine_anchors.append(start + end)
            elif start_count != 1 or end_count != 1:
                raise ValueError('机器锚点必须成对且恰好一次: ' + str(anchor))
        if missing_machine_anchors:
            hidden_machine_anchors = '<div class="machine-anchor-bank" hidden aria-hidden="true">' + ''.join(missing_machine_anchors) + '</div>'
    ...
    return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>' + e(model["title"]) + '</title><style>' + _page_css(contract, route) + '</style></head><meta name="review-date" content="' + e(model["d"]) + '"><meta name="review-schema-version" content="' + str(model["schema_version"]) + '">'+ '<body data-route="' + e(route) + '" data-d="' + e(model["d"]) + '" data-schema-version="' + str(model["schema_version"]) + '" data-template-version="' + e(contract["template_version"]) + '">' + nav + '<div class="wrap">' + body + hidden_evidence_anchors + hidden_machine_anchors + contract["visual"]["foot"] + '</div>' + ('' if GOLDEN_INDEX_ON else VISUAL_JS) + '</body></html>\n'

def input_files(root, d):
    """候选发布冻结清单: 概览页黄金骨架渲染器也是产出物的一部分(与发布门禁同源)。"""
    return ["module_golden_index.py"]
