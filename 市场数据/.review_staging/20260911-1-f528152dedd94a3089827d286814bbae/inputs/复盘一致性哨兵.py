...
for p in pages:
    ...
    if _ri>0:
        ...
        _expected_items = ['hero', 'kpi'] if p == 'cycle' and '<section id="' not in h else ['hero','kpi','kpi','kpi','kpi']
...
if os.path.isfile(_lhb_checker):
    _cp = subprocess.run(
        [sys.executable, _lhb_checker, d, '--page', 'lhb.html'],
        cwd=R, env=dict(os.environ, LHB_SITE_ROOT=SITE), capture_output=True, text=True, encoding='utf-8',
    )
    if _cp.returncode != 0:
